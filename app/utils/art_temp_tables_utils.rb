...
module ArtTempTablesUtils
  ...
  def prepare_cohort_tables
    ...
    drop_temp_other_patient_types unless count_table_columns('temp_other_patient_types') == 2
    create_temp_register_start_date_table unless check_if_table_exists('temp_register_start_date')
    drop_temp_register_start_date_table unless count_table_columns('temp_register_start_date') == 3
    create_temp_order_details unless check_if_table_exists('temp_order_details')
    drop_temp_order_details unless count_table_columns('temp_order_details') == 3
    create_art_start_date unless check_if_table_exists('temp_art_start_date')
    drop_art_start_date unless count_table_columns('temp_art_start_date') == 3
    create_temp_patient_tb_status unless check_if_table_exists('temp_patient_tb_status')
    drop_temp_patient_tb_status unless count_table_columns('temp_patient_tb_status') == 3
    create_temp_latest_tb_status unless check_if_table_exists('temp_latest_tb_status')
    drop_temp_latest_tb_status unless count_table_columns('temp_latest_tb_status') == 3
    create_tmp_max_adherence unless check_if_table_exists('tmp_max_adherence')
    drop_tmp_max_adherence unless count_table_columns('tmp_max_adherence') == 3
    create_temp_pregnant_obs unless check_if_table_exists('temp_pregnant_obs')
    drop_temp_pregnant_obs unless count_table_columns('temp_pregnant_obs') == 4
    create_temp_patient_side_effects unless check_if_table_exists('temp_patient_side_effects')
    drop_temp_patient_side_effects unless count_table_columns('temp_patient_side_effects') == 3
  ...
  def prepare_outcome_tables
    [false, true].each do |start|
      ...
      unless count_table_columns("temp_patient_outcomes#{start ? '_start' : ''}") == 7
      ...
      unless count_table_columns("temp_max_drug_orders#{start ? '_start' : ''}") == 4
      ...
      unless count_table_columns("temp_min_auto_expire_date#{start ? '_start' : ''}") == 6
      ...
      drop_temp_current_state(start:) unless count_table_columns("temp_current_state#{start ? '_start' : ''}") == 7
    end
  end

  def prepare_maternal_tables
    create_temp_maternal_status unless check_if_table_exists('temp_maternal_status')
    unless count_table_columns('temp_maternal_status') == 3
  ...
  def create_temp_other_patient_types
    ActiveRecord::Base.connection.execute <<~SQL
      CREATE TABLE temp_other_patient_types (
        patient_id INT(11) NOT NULL, site_id INT(11) NOT NULL,
        PRIMARY KEY (patient_id)
      )
    SQL
  end
  ...
  def truncate_cohort_tables
    ActiveRecord::Base.connection.execute("DELETE FROM temp_cohort_members #{site_filter(table_name: 'temp_cohort_members', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_earliest_start_date #{site_filter(table_name: 'temp_earliest_start_date', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_other_patient_types #{site_filter(table_name: 'temp_other_patient_types', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_register_start_date #{site_filter(table_name: 'temp_register_start_date', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_order_details #{site_filter(table_name: 'temp_order_details', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_art_start_date #{site_filter(table_name: 'temp_art_start_date', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_patient_tb_status #{site_filter(table_name: 'temp_patient_tb_status', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_latest_tb_status #{site_filter(table_name: 'temp_latest_tb_status', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM tmp_max_adherence #{site_filter(table_name: 'tmp_max_adherence', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_pregnant_obs #{site_filter(table_name: 'temp_pregnant_obs', clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_patient_side_effects #{site_filter(table_name: 'temp_patient_side_effects', clause: 'WHERE')}")
  end
  ...
  def truncate_outcome_tables(start: false)
    ...
    ActiveRecord::Base.connection.execute("DELETE FROM temp_patient_outcomes#{name} #{site_filter(table_name: "temp_patient_outcomes#{name}", clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_max_drug_orders#{name} #{site_filter(table_name: "temp_max_drug_orders#{name}", clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_min_auto_expire_date#{name} #{site_filter(table_name: "temp_min_auto_expire_date#{name}", clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_max_patient_state#{name} #{site_filter(table_name: "temp_max_patient_state#{name}", clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_current_state#{name} #{site_filter(table_name: "temp_current_state#{name}", clause: 'WHERE')}")
    ActiveRecord::Base.connection.execute("DELETE FROM temp_current_medication#{name} #{site_filter(table_name: "temp_current_medication#{name}", clause: 'WHERE')}")
  end
end
