    ...
    module Ahd
      ...
      class Monthly
        ...
        def build_all_test_reports(patient_id, tests)
          ...
          build_test_results_report(patient_id, tests, 'HIV Viral load', vl_indicators,
                                    'hiv_viral_load') do |modifier, value|
        ...
        def build_who_stage(patient_id, who_stage)
          ...
          stage_groups.each do |key, stages|
            if ConceptName.where(name: stages).pluck('concept_id').include?(who_stage)
              report[indicator][key] << patient_id
            end
          end
        end
        ...
        def build_indicators(indicator, keys)
          report[indicator] ||= {}
          keys.each_with_object({}) do |key, _hash|
