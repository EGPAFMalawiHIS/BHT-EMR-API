    ...
    def print1
      ...
      out.each do |key, element|
        ...
        if element == @date.to_date.strftime("%d/%b/%Y")
          ...
          vis["position_presentation"] = "#{pres} #{posi}"
    ...
    def detailed_obstetric_history_label(date = Date.today)
      ...
      (1..(@obstetrics.length + 1)).each do |pos|
        ...
        if pos <= 3
          ...
          label1json["year"] = (@obstetrics[pos] ? (@obstetrics[pos]["YEAR OF BIRTH"]) : "")
          ...
          label2json["year"] = (@obstetrics[pos] ? (@obstetrics[pos]["YEAR OF BIRTH"]) : "")

          if @place.length < 8
            label2.draw_text(@place, 111, ((55 * (pos - 3)) - 13), 0, 2, 1, 1, false)
            label2json["place"] = @place
          else
            @place = paragraphate(@place)
          ...
          if @birt_weig.length < 6
            ...
            label2json["birt_weig"] = @birt_weig
          else
          ...
          label2json["alive"] = (@obstetrics[pos] ? (@obstetrics[pos]["ALIVE"]) : "")
          ...
          label3json["year"] = (@obstetrics[pos] ? (@obstetrics[pos]["YEAR OF BIRTH"]) : "")

          if @place.length < 8
            label3.draw_text(@place, 111, ((55 * (pos - 8)) - 13), 0, 2, 1, 1, false)
            label3json["place"] = @place
          else
            @place = paragraphate(@place)
          ...
          if @birt_weig.length < 6
            ...
            label3json["birt_weig"] = @birt_weig
          else
            @birt_weig = paragraphate(@birt_weig, 4)
          ...
          label3json["alive"] = (@obstetrics[pos] ? (@obstetrics[pos]["ALIVE"]) : "")
