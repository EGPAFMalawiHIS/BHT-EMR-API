    ...
    module Moh
      class CancerSuspects
        ...
        def screened
          ...
          suspected = concept 'Suspect cancer'

          obs = Observation.where("obs.concept_id = ? AND p.gender IN(?)
					AND obs_datetime BETWEEN ? AND ? AND value_coded = ?", screening_result.concept_id,
                                  %w[F Female], @start_date, @end_date, suspected.concept_id)\
